    ...
    public PromotionGoodsResp SenJdApiGoods(List<String> skuIds, Map<String, String> parse) throws Exception {
        ...
        if (response.getQueryResult().getData().length > 0){
            ...
            if (!datum.getGoodsName().isEmpty()){
                StringBuffer materiaUrl = new StringBuffer("https://item.jd.com/");
                materiaUrl.append(datum.getSkuId());
                materiaUrl.append(".html");
                String resJSON = dtkApiService.SenJdApiConvertUrl(materiaUrl.toString(),Long.valueOf(parse.get("openId")));
